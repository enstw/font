...
以 **LXGW WenKai TC / LXGW WenKai Mono TC** 為基底，將 donor 字型移植進去：

| 來源 | 負責範圍 |
|------|---------|
| **JetBrains Sans / JetBrainsMono Nerd Font Mono**（優先） | donor 字型涵蓋的所有字元，一律優先覆蓋 WenKai TC |
| **LXGW WenKai TC**（補充） | 僅保留 donor 沒有的字元，主要為 CJK、假名、全形標點等 |

規則只有一條：**donor 有的字以 donor 為準，其餘由 WenKai TC 補足。**

目前產物對應如下：

| 產物 | CJK 基底 | Donor 來源 |
|------|---------|------------------|
| ENS Font | LXGW WenKai TC | JetBrains Sans + Nerd Fonts (patched) |
| ENS Font Mono | LXGW WenKai Mono TC | JetBrainsMono Nerd Font Mono |

---

## 字重與樣式說明

LXGW WenKai TC / LXGW WenKai Mono TC 皆提供 Regular / Bold / Light 三種字重。ENS Font 與 ENS Font Mono 對應如下：

| 產物 | 字重 | WenKai TC CJK 來源 | Donor 來源 |
|------|------|-------------------|------------------|
| ENS Font | Light | Light | JetBrains Sans-Light |
| ENS Font | Regular | Regular | JetBrains Sans-Regular |
| ENS Font | Bold | Bold | JetBrains Sans-Bold |
| ENS Font Mono | Light | Light | JetBrainsMonoNerdFontMono-Light |
| ENS Font Mono | Regular | Regular | JetBrainsMonoNerdFontMono-Regular |
| ENS Font Mono | Bold | Bold | JetBrainsMonoNerdFontMono-Bold |
...
# 1. 下載 LXGW WenKai TC
curl -fL "https://github.com/lxgw/LxgwWenKaiTC/releases/latest/download/lxgw-wenkai-tc-v1.521.zip" -o /tmp/lxgw-wenkai-tc.zip
unzip -j /tmp/lxgw-wenkai-tc.zip "*.ttf" -d fonts/wenkai/
curl -fL "https://github.com/lxgw/LxgwWenKaiTC/releases/latest/download/lxgw-wenkai-mono-tc-v1.521.zip" -o /tmp/lxgw-wenkai-mono-tc.zip
unzip -j /tmp/lxgw-wenkai-mono-tc.zip "*.ttf" -d fonts/wenkai/
